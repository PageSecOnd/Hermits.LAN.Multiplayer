...
加入界面打开期间，Mod 每秒发送一次很小的发现探测包。主机直接响应后，会在房间列表中显示当前人数、游戏模式、往返延迟、滚动丢包率及综合连接质量。点击人数未满的房间即可直接加入。

自动发现使用 UDP `33770` 端口。如果防火墙拦截了发现流量，或者当前虚拟局域网不转发广播，手动输入地址仍可正常使用。主机只响应来自本机所连接 IPv4 子网的发现请求。
